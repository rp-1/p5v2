...
function ViewModel() {
    ...
    self.numRestaurants = ko.observable(0);
    self.map = null;
    self.listDropSpeed = 400;
    self.dropdownThreshhold = 480;
    self.infoWindowOffsetX = -130;
    self.infoWindowOffsetY = -150;
    ...
    self.init = function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                self.numRestaurants(self.places().length);
            },
        ...
        // When in mobile, we want our places list to drop down whenever
        // focus is given to search bar.
        $("#searchbar").focus(function() {
            if($("#places-area").height() > 0) {
                $("#places-area").slideDown(self.listDropSpeed);
            }
        });
    };

    // Called when an li item is clicked or marker is selected directly
    // If called by an li item click, event will be an object. Otherwise it will be null
    self.placeClicked = function(placeClicked, event) {
        ...
        // if we're mobile size, make list animate off screen to reveal map
        // If event is not null, we know this function was called as the result of
        // user clicking an item in the list, as opposed to clicking a marker
        if(event && $(window).width() <= self.dropdownThreshhold) {
            $("#places-area").slideUp(self.listDropSpeed);
        }
    ...
    self.searchString.subscribe(function(value) {
            ...
            for (var i = 0; i < self.places().length; i++) {
                ...
                for(var x = 0; x < term_array.length; x++) {
                    ...
                    (full_name.substr(0, search_string.length) === search_string) ) {
                        ...
                        self.numRestaurants(self.numRestaurants() + 1);
                        break;
                    }
                }
        ...
        // If we're here that means the searchbar is completely clear
        // so show entire default list of restaurants
        } else {
            for(var j = 0; j < self.places().length; j++) {
                self.places()[j].marker.setMap(self.map);
                self.places()[j].isActive(true);
                self.places()[j].isSelected(false);
            }
            self.numRestaurants(self.places().length);
        }
    ...
    self.showInfo = function(place) {
        ...
        var offsetX = self.infoWindowOffsetX;
        var offsetY = self.infoWindowOffsetY;

        // If we're on a small device, don't move marker to the right
        if($(window).width() <= self.dropdownThreshhold) {
            offsetX = 0;
            offsetY -= 50;
        }

        self.map.setCenterWithOffset(place.marker.getPosition(), offsetX, offsetY);
...
// Routine to resize map with offset values in pixels.
// Credit: http://w3facility.org/question/how-to-offset-the-center-of-a-google-maps-api-v3-in-pixels/
